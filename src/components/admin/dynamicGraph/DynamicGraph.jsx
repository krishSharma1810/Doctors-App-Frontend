...
const DynamicGraph = () => {
    ...
    const doctorAndPatientCount = async () => {
        ...
        try {
            const response = await axios.get(`https://doctors-app-backend-8uvd.onrender.com/getTotalPatientByDoctor`, { headers: { 'Authorization': `${token}` } });
    ...
    const dateAndPatientCount = async () => {
        const token = localStorage.getItem('authToken');
        try {
            const response = await axios.get(`https://doctors-app-backend-8uvd.onrender.com/getTotalPAtientByDate`, { headers: { 'Authorization': `${token}` } });
