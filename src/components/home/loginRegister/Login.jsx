...
const Login = () => {
  ...
  const requestOtp = async (e) => {
    ...
    setOtpFlag(true);
    axios.post("https://doctors-app-backend-8uvd.onrender.com/sendotp", { email: registrationData.email })
      .then((response) => {
        setOtpFlag(true);
  ...
  const loginUsingPassword = async (e) => {
    ...
    axios.post("https://doctors-app-backend-8uvd.onrender.com/login", registrationData).
      then(response => {
  ...
  const loginUsingOTP = async (e) => {
    ...
    axios.post("https://doctors-app-backend-8uvd.onrender.com/verifyOtp", registrationData)
      .then((response) => {
        if (response.data.userData.role === "admin") {
          navigate(`/admin/admindetails`);
        }
